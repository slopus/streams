...
Produce jobs with the same normal write as Model A (`POST /v0/topics/jobs`). A non-queue topic rejects every claim/ack/nack/extend/work endpoint with `409 not_a_queue`.
...
```bash
curl -X POST localhost:4000/v0/topics/jobs/claim \
  -H 'content-type: application/json' \
...
```bash
curl -X POST localhost:4000/v0/topics/jobs/ack \
  -H 'content-type: application/json' \
...
```json
# →
{ "topic": "jobs", "acked": 1, "skipped": [], "ready": 0, "in_flight": 0,
...
```bash
curl -X POST localhost:4000/v0/topics/jobs.dlq/diff -d '{ "from_seq": 0, "include_meta": true }'
```
...
```bash
curl -N "localhost:4000/v0/topics/jobs/work?node=worker-1&max=8" \
...
id: 1
event: job
data: {"topic":"jobs","$seq":1,"lease_id":"lease_7f3a9c","deadline":1748470030000,"$ts":1748470000123,"$tag":"tenant42:job-9001","deliveries":1,"data":{"url":"s3://b/a.png"}}
