...
```bash
curl -X POST $STREAMS/v0/topics/transcode/diff \
  -H 'content-type: application/json' \
...
curl -X POST $STREAMS/v0/topics/transcode/delete \
  -H 'content-type: application/json' \
...
```bash
curl -X POST $STREAMS/v0/topics/transcode/delete -d '{ "before_seq": 480000 }'
```
...
```bash
curl -X POST $STREAMS/v0/topics/transcode/claim \
  -H 'content-type: application/json' \
...
```bash
curl -X POST $STREAMS/v0/topics/transcode/ack \
  -H 'content-type: application/json' \
...
```bash
curl -X POST $STREAMS/v0/topics/transcode.dlq/diff -d '{ "from_seq": 0, "include_meta": true }'
```
...
```bash
curl -N "$STREAMS/v0/topics/transcode/work?node=transcode-1&max=8" \
