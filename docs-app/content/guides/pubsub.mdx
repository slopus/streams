...
Examples use `$STREAMS` as the base URL — see the [Quickstart](/getting-started/quickstart).
...
```bash
curl -X PUT $STREAMS/v0/topics/notifications:web -d '{ "ttl_ms": 5000, "cap_records": 10000 }'
curl -X PUT $STREAMS/v0/topics/notifications:mobile -d '{ "ttl_ms": 5000, "cap_records": 10000 }'
...
```bash
curl -X POST $STREAMS/v0/topics/notifications \
  -H 'content-type: application/json' \
  -d '{ "records": [
