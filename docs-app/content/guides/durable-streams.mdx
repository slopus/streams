...
streams tracks two floors per topic (the [dual watermark](/guarantees/tombstones)). `evict_floor` is advanced **only** by involuntary cap eviction and TTL expiry — and it is the **sole** trigger for a tombstone. `earliest_seq` (the first live record) is advanced by eviction, TTL, *and* voluntary deletion.
...
```bash
curl -X POST localhost:4000/v0/topics/ledger \
  -H 'content-type: application/json' \
...
```bash
curl -X POST localhost:4000/v0/topics/ledger/diff \
  -H 'content-type: application/json' \
...
curl -X POST localhost:4000/v0/topics/ledger/diff -d '{ "from_seq": 480123, "limit": 1000 }'

# Full reprocess from the beginning of retained history.
curl -X POST localhost:4000/v0/topics/ledger/diff -d '{ "from_seq": 0, "limit": 1000 }'
```
...
curl -X POST localhost:4000/v0/topics/ledger/delete -d '{ "before_seq": 480000 }'
```
