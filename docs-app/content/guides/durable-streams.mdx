...
```bash
curl -X POST $STREAMS/v0/topics/ledger \
  -H 'content-type: application/json' \
...
```bash
curl -X POST $STREAMS/v0/topics/ledger/diff \
  -H 'content-type: application/json' \
...
curl -X POST $STREAMS/v0/topics/ledger/diff -d '{ "from_seq": 480123, "limit": 1000 }'

# Full reprocess from the beginning of retained history.
curl -X POST $STREAMS/v0/topics/ledger/diff -d '{ "from_seq": 0, "limit": 1000 }'
```
...
curl -X POST $STREAMS/v0/topics/ledger/delete -d '{ "before_seq": 480000 }'
```
