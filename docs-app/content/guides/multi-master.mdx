...
**Multi-master** means several symmetric nodes that each write events *and* each consume the union of everyone's events — without a node ever receiving back what it produced, and without a record looping forever around the topology. streams makes this safe by construction with two complementary layers: **node loop-prevention** (content-level — a node never delivers its own records to itself) and **router cycle control** (topology-level — a hop-cap stops unbounded forwarding). The contract is simple: every node sets its own `node` id on writes *and* presents it as a filter on reads, and routers preserve `$node` verbatim through every forward.

Get those two halves right and an A↔B↔C mirror is echo-free and loop-free with no distributed consensus, no clock sync, no leader — just append-only logs and forwarding rules on each box.
...
<Callout type="important">
  ...
  origin `$node` unchanged through every router hop, so when it arrives back at a box that X
...
- **Node loop-prevention (correctness).** Every record may carry an origin `$node`, stamped by the writer, **immutable**, and **preserved verbatim** through every router forward. A read that presents a matching `node` drops every record whose `$node` equals it, byte-exact, before delivery. So node A writes `$node=A`; routers fan it to B's and C's boxes; B and C read with their own node filters and receive A's record but never re-emit it as their own. A record that loops back to a box A reads is filtered at A's read. This makes delivery echo-free.
...
## Recipe — a two-node A↔B mirror

The simplest multi-master: two nodes, each with a shared box, mirrored both ways so each node sees the other's writes.

```text
   node A                                node B
   write $node=A ──► ┌────────┐  router A→B  ┌────────┐ ◄── write $node=B
                     │ box on │ ───────────► │ box on │
   read node=A   ◄── │  A     │ ◄─────────── │  B     │ ──► read node=B
                     └────────┘  router B→A  └────────┘
```
...
```bash
# On node A:
curl -X POST localhost:4000/v0/boxes/events \
  -H 'content-type: application/json' \
  -d '{ "node": "A",
        "records": [ { "data": { "evt": "user.created", "id": 42 } } ] }'

# On node B:
curl -X POST localhost:4000/v0/boxes/events \
  -H 'content-type: application/json' \
  -d '{ "node": "B",
        "records": [ { "data": { "evt": "order.placed", "id": 7 } } ] }'
```

The origin `$node` (`A` or `B`) is now immutable and rides through every forward.

### Create symmetric routers between the nodes

One router each direction. Because A→B and B→A together form a directed cycle, set `allow_cycle: true` on both so creation isn't rejected with `409 router_cycle`; the route then uses hop-cap loop-breaking. `preserve_node: true` (the default) is what carries `$node` across the forward — **do not** set it false, or loop-prevention breaks.

```bash
# Router forwarding A's box into B's box:
curl -X PUT localhost:4000/v0/routers/a-to-b \
  -H 'content-type: application/json' \
  -d '{ "source": "events", "dest": "events-mirror", "allow_cycle": true }'

# Router forwarding B's box back into A's box (symmetric):
curl -X PUT localhost:4000/v0/routers/b-to-a \
  -H 'content-type: application/json' \
  -d '{ "source": "events-mirror", "dest": "events", "allow_cycle": true }'
```

```json
# →
{ "router": "a-to-b", "created": true,
  "source": "events", "dest": "events-mirror",
  "preserve_node": true, "preserve_tag": true, "filter": null, "allow_cycle": true,
  "performance": { "server_total_ms": 0.20 } }
```

<Callout type="info">
  In a real deployment each "box" lives on a *separate streams process* on a different
  machine; a router's `dest` would name a box reached through that node's HTTP endpoint. The
  semantics are identical — `$node` preservation and the hop-cap work the same whether the
  hops are in one process or across the network. (streams is single-machine per process; the
  mirror is built by pointing routers at peer instances.)
</Callout>
...
```bash
# On node A — read everything, drop records A produced:
curl -X POST localhost:4000/v0/boxes/events/diff \
  -H 'content-type: application/json' \
  -d '{ "from_seq": 0, "node": "A" }'
```
...
{ "box": "events",
  "records": [
    { "$seq": 12, "$ts": 1748470000500, "$node": "B",
      "data": { "evt": "order.placed", "id": 7 } }
  ],
...
A's own records (`$node: "A"`) were filtered out — but the cursor still advanced **past** them. So `caught_up`, not `records.length`, is the "no more right now" signal.
...
<Callout type="warning">
  ...
  reading a box full of its own events would otherwise loop forever. Always rely on
...
curl -X POST localhost:4000/v0/watch \
  -H 'content-type: application/json' \
  -d '{ "node": "A", "boxes": { "events": { "from_seq": 0 } } }'
# → returns a wid; GET it to stream. Records with $node "A" are never delivered to A.
```
...
- **Deletes and filters are per-box.** A delete on one node's box does **not** propagate through routers — a copy may already have been forwarded. To remove a record everywhere, delete it on each box (or design idempotent consumers). See [Routers](/api/routers).
...
- **Per-box durability still applies independently.** Each box in the mirror chooses its own [durability class](/guarantees/durability); a forward honors the *destination* box's class (a `memory` dest persists no copy).
