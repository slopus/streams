...
The simplest multi-master: two logical nodes (origin labels), each with its own topic **in the same server**, mirrored both ways with local routers so each node sees the other's writes. (Nodes are content labels, not machines — streams is single-server.)

```text
   node A (label)                        node B (label)
   write $node=A ──► ┌────────┐  router A→B  ┌────────┐ ◄── write $node=B
                     │ topic-A   │ ───────────► │ topic-B  │
   read node=A   ◄── │ (local) │ ◄─────────── │(local) │ ──► read node=B
                     └────────┘  router B→A  └────────┘
```
...
```bash
# On node A:
curl -X POST localhost:4000/v0/topics/events \
...
```bash
# Router forwarding A's topic into B's topic:
curl -X PUT localhost:4000/v0/routers/a-to-b \
  -H 'content-type: application/json' \
  -d '{ "source": "events", "dest": "events-mirror", "allow_cycle": true }'

# Router forwarding B's topic back into A's topic (symmetric):
...
<Callout type="warning">
  **This is a single-server, local pattern.** A "node" here is a *logical origin label*
  ...
  streams does **not** do multi-server replication or HA. The mirror is built from local topics
...
curl -X POST localhost:4000/v0/topics/events/diff \
  -H 'content-type: application/json' \
  -d '{ "from_seq": 0, "node": "A" }'
```
...
curl -X POST localhost:4000/v0/watch \
  -H 'content-type: application/json' \
  -d '{ "node": "A", "topics": { "events": { "from_seq": 0 } } }'
