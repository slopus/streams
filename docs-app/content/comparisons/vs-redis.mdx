...
- **`ephemeral`** — resident-only records: queryable while the process runs, with durable
  config and monotonic seqs, but intentionally empty after restart.
- **`memory`** — _disk-like but best-effort_: the same group-committed WAL path as `disk`, fully
  ...
  config always persists). Fastest disk-like class (never fsync-gated).
...
- [Durability classes](/guarantees/durability) — `ephemeral` / `memory` / `disk` / `fsync` per topic.
