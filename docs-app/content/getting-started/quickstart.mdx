---
title: Quickstart
description: Run streams and write, read, watch, fan out, and delete your first events over plain HTTP and JSON — no client library required.
---
...
<Callout type="info">
  ...
  and that `STREAMS` points at the server. See [Install & Run](/getting-started/installation)
...
# auth disabled (dev mode); WAL + segments + snapshots under ./streams-data.
cargo build --release
...
```bash
curl -X PUT $STREAMS/v0/routers/orders-to-fulfillment \
  -H 'content-type: application/json' \
