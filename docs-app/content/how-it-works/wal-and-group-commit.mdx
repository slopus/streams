...
The **write-ahead log** is the durability boundary of topics: an acknowledged write to a
durable topic is a complete, checksum-valid frame on disk, and recovery loses only data that
...
```text
 off  size  field
   ...
   6    4   topic_id      u32   interned numeric topic id (name<->id in the meta store)
...
- **`topic_id` is an interned `u32`,** not the string name, so frames stay small; the name↔id
...
Appends funnel into the topic's **WAL-shard writer thread**, each shard fed by its own bounded
...
Durability is per-topic across **three** commit classes, and each shard writer collapses them
...
wait happens. The per-topic lock is held only for the microsecond-scale work; the slow part
runs without it.

1. **Under the per-topic lock (microseconds):** assign the seq and enqueue the frame on the
   WAL channel. This is short, so it does not serialize unrelated writers for long.
2. **Off the lock:** the writer waits for the group `fdatasync`. Because the per-topic lock is
...
- **`fsync` topic:** an acked write waited for `fdatasync`, so it is a complete checksum-valid
  frame ⇒ **never lost**.
- **`disk` topic:** an acked write is in the page cache but may not be fsynced; a power loss
  can drop the un-fsynced tail — the documented fast-path tradeoff, surfacing on read as an
  ordinary eviction-style gap.
- **`memory` topic:** written to the WAL on the **same** group-committed path as `disk`, but with
  no durability guarantee — after a restart its records **may survive or be lost** (best-effort,
  gradual recovery); the topic config always persists.
...
- [Segments & Snapshots](/how-it-works/segments-and-snapshots) — how WAL frames are
  checkpointed into the long-term per-topic store.
- [Crash Recovery](/how-it-works/recovery) — snapshot load, WAL replay, and torn-tail
  truncation.
- [Durability](/guarantees/durability) — the `ephemeral` / `memory` / `disk` / `fsync` classes from the
  consumer's side.
