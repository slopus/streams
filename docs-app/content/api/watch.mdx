...
|---|---|
| `200` | Stream opened. |
| `401 unauthorized` | A bearer or `?token=` *was* presented but does not match the key that created the `wid`. (Omitting the bearer is fine — the `wid` is the capability when auth is off.) |
...
`reason` ∈ `cap` | `ttl` | `mixed` | `recreated`, plus `from_seq_too_old` — emitted
