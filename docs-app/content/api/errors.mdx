...
- **`401` vs `403`** — `401` means no/invalid token; `403` means the token authenticates
  but lacks the required [scope](/deployment/security), or names a topic/router outside its
...
- **`409 not_a_queue`** — a [queue](/api/queues) endpoint (`claim`/`ack`/`nack`/`extend`/
  `work`) was called on a plain `"log"` topic.
- **`409 topic_exists_incompatible`** — a `PUT` tried to change a topic's immutable `type`
  (`log` ↔ `queue`).
...
topics, routers, watch sessions, SSE connections, in-flight requests per key, or total
