...
- **`401` vs `403`** — `401` means no/invalid token; `403` means the token authenticates
  ...
  [watch](/api/watch), `401` is returned only when a bearer or `?token=` *was* presented
  but does not match the key that created the `wid` — omitting the bearer entirely is fine,
  since the `wid` is the capability.
...
| `wal_append_ms` | when relevant | Time spent enqueuing the WAL frame. `0` on `memory` boxes. |
