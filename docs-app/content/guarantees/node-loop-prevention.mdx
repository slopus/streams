...
```bash
# worker-eu-1 reads the box it also writes to; its own records are filtered out.
curl -X POST localhost:4000/v0/boxes/jobs/diff \
  -H 'content-type: application/json' \
  -d '{ "from_seq": 0, "node": "worker-eu-1" }'
```
...
Without this, a node reading a box full of its own events would loop forever, unable to
...
[SSE watch](/api/watch): `node` on the watch session is applied to every box it watches, and
...
2. Routers fan that record out to boxes that B and C watch. The forwarded copies still carry
...
4. If a forwarded copy loops back to a box that **A** reads, A's own filter drops it.
...
Node loop-prevention is enabled per box via the `dedupe_node` config field, which defaults
to `true`. Set `dedupe_node: false` if you genuinely want echoes — a box where a node *does*
receive back its own records.

```bash
curl -X PUT localhost:4000/v0/boxes/echo \
  -H 'content-type: application/json' \
  -d '{ "dedupe_node": false }'
```

With `dedupe_node: false`, the reader's `node` filter is ignored on that box and every
record is delivered regardless of `$node`. This is rarely what you want; it exists so a box
