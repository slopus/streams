...
- **`earliest_seq`** — the seq of the **first currently-live record**: not eviction-reclaimed,
  not TTL-expired, and **not deleted**. It is reported in topic state and in every `diff`
  response. If no live record exists it is `head_seq + 1`. It is **monotonically
  non-decreasing** over a topic instance's life — eviction, TTL expiry, **and deletion** all
...
event: tombstone
data: {"topic":"pageviews","reason":"cap","gap_from":80000,"gap_to":83000,"earliest_seq":83001,"head_seq":88130}
```

The frame's `id:` already advances the topic cursor to `gap_to`, so a reconnect resuming after
