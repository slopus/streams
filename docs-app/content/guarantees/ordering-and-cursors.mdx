...
<Callout type="info">
  ...
  beyond any single topic's lifetime, so no string encoding is needed. Clients should
...
- A tail / only-new cursor is `from_seq = head_seq` at subscription — like Redis `$`. You
  can read this off topic state as `next_seq - 1`.

There is no opaque continuation token on topic reads. The client **owns** its cursor; on the
...
skips: otherwise a consumer reading a topic full of its own (filtered) events would loop
...
value; the topic config always persists. See [Durability Classes](/guarantees/durability).
