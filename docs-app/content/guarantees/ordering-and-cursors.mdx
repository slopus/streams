...
A `memory` box is a special case: on restart it reappears **empty**, `head_seq` resets to
`0`, but the box config persists — so a stale consumer reading from a high cursor is handled
by the recreate path below.
